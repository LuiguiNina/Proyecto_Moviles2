...
class _MapaBasePageState extends State<MapaBasePage> {
  ...
  Future<Set<Marker>> _createMarkers(List<QueryDocumentSnapshot> negocios) async {
    ...
    for (var negocio in negocios) {
      if (await _checkSearchMatch(negocio)) {
        ...
        if (searchQuery.isNotEmpty) {
          try {
            ...
            if (cartaRef.docs.isNotEmpty && 
              ...
              if (productosFiltrados.isNotEmpty) {
                var precios = productosFiltrados.map((e) => (e['precio'] as num).toDouble()).toList();
                ...
                for (var producto in productosFiltrados) {
                  ...
                  if (productosFiltrados.length == 1) {
                    ...
                    markerHue = price == minPrice ? BitmapDescriptor.hueGreen :
                               price == maxPrice ? BitmapDescriptor.hueRed :
                      ...
                      onTap: () {
                        setState(() {
                          if (selectedNegocioId == negocio.id) {
                            ...
                            selectedNegocioLocation = LatLng(geoPoint.latitude, geoPoint.longitude);
                          }
                        });
                      },
          ...
          if (negocio['nombre'].toString().toLowerCase().contains(searchQuery.toLowerCase()) &&
              !newMarkers.any((marker) => marker.markerId.value.startsWith(negocio.id))) {
            newMarkers.add(
              Marker(
                          ...
                          return GestureDetector(
                            ...
                            child: Container(
                              ...
                              child: Column(
                                children: [
                                  Image.network(
                                    (tipoCocina.data() as Map<String, dynamic>)['imagen'] ?? '',
                                    width: 40,
                                    height: 40,
                                  ...
                                  const SizedBox(height: 4.0),
                                  Text((tipoCocina.data() as Map<String, dynamic>)['nombre'] ?? ''),
                                ],
                              ),
                            ),
                          );
